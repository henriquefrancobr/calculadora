...
Vn       ;;verifica se o numero eh negativo
...
In ;;imprime numero forma negativa
      PUSH {R1}
      MOV R1, #'-'
      PUSH {LR}
      BL Write
...
Putt: ;; funcao de digitar
        PUSH {LR}
        LDR R3, =putt ; inicio
        MOV R5, #16

LoopPutt ;; loop para digitar
        LDR R1, [R3] ; le^
        BL Write 
        ADD R3, #1
        SUBS R5, #1
...
Impossivel: ;;msg que nao possivel fazer a conta
        LDR R3, =impossivel ; inicio
        MOV R5, #28 
...
Write: ;;Digita
        ...
        PUSH {R0}
        MOV R0, #0x2000 ; atraso em milissegundos
        PUSH {LR}
        BL SW_delay
        POP {LR}
        POP {R0}
...
;; seção de constantes em ROM
            ...
            DATA
putt        DC8  "Digite a conta:"
impossivel  DC8  "Impossivel dividir por zero"
lf          DC8  00001010b
cr          DC8  00001101b
